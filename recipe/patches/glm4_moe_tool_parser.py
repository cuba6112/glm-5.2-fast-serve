    ...
    def extract_tool_calls_streaming(
        ...
        for i, (inner_text, is_complete) in enumerate(regions):
            ...
            if not tool_name and is_complete:
                # GLM-4.7 zero-argument call: <tool_call>name</tool_call>
                # has no newline or <arg_key> after the name, so
                # _extract_tool_name_from_region returns None and the call
                # would be silently dropped (never emitted to the client).
                candidate = inner_text.strip()
                if candidate and "<" not in candidate:
                    tool_name = candidate.split()[0]
            if not tool_name:
                break
